...
res_fn_defn(read_json_file, (const char* filename), json, std::string) {
...
res_fn_defn(validate_arithmetic_expression, (const json& j), bool, std::string) {
    if(j.is_number())
        return OK(true);
    if(j.is_string())
        return OK(true);
...
res_fn_defn(evaluate_arithmetic_expression,
       (const json& expr,
...
res_fn_defn(validate_rewrite_target, (const json& j), bool, std::string) {
    if(j.is_string())
        return OK(true);
    if(j.is_array()) {
...
res_fn_defn(parse_rewrite_target,
...
res_fn_defn(parse_rewrite_rules,
       (const json& j, const std::map<std::string, float>& globals),
...
res_fn_defn(parse_tree, (const json& j), ParsedTree, std::string) {
	if(!j.is_object())
...
res_fn_defn(from_json, (const json& j),
...
res_fn_defn(from_json_file, (const char* json_filename),
	   std::map<std::string, ParsedTree>, std::string) {
